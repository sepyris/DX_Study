...
TutorialScene::TutorialScene()
{
	...
	// 그 삼각형을 출력하기 위해선 해당 삼각형을 구성하는 정점들이 필요

	// 그래서 그 정점 = 꼭지점을 아래와 같이 미리 선언해두는 것
	Vertex vertices[] =
	{
		...
		Vertex(+0.1f, +0.1f, 1.0f, 0.0f, 0.0f),
		Vertex(+0.1f, +0.8f, 0.0f, 1.0f, 0.0f),
		Vertex(+0.8f, +0.8f, 0.0f, 0.0f, 1.0f),
		// DX의 기본값으로는
		// 화면 전체의 좌표계를
		...
		// 기본적으로는 3D 그래픽을 출력하는 원리를 이용해서 화면을 구성 중
		...
		// 그래서 지금 이렇게 그림을 그리기 위해 구성한 꼭지점들이
		// 시계방향을 그리고 있지 않다면 출력되지 않음
		// 이에 대한 자세한 개념은 3D를 다루게 되는 심화반 쪽에서 배우시게 될 것
		// 지금은 "꼭지점은 반드시 시계방향을 그려야만 한다"라고 기억하면 OK
...
// 이 씬이 사용중일 떄, 메인 루프가 돌아갈 때마다 이 함수를 통해 화면에 출력
{
	...
	Device::Get()->GetDeviceContext()
		->IASetInputLayout(VS->GetInputLayout());
	...
	// D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST :
	// 삼각형을 그리라는 뜻이나,
	// 그 삼각형을 그릴 때 무조건 꼭지점을 3개 단위로 끊어서 이용할 것
	// 입력된 정점들을 한 번에 삼각형을 출력할 때 앞에서부터 3개씩,
	// 중복되는 정점 없이 이용할 것

	// D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST :
	// 마찬가지로 삼각형을 그리라는 뜻이지만,
	// 마지막에 사용했던 꼭지점 중 뒤쪽의 2개를 재활용해서
	// 삼각형을 그리도록 하는 모드
	// ex| 1,2,3,4라는 4개 정점을 이용해 그렸을 때
	// TRIANGLELIST는 1,2,3 3개만 이용해 그리고 끝나지만
	// TRIANGLESTRIP은 1,2,3으로 그린 뒤에 4,3,2를 이용해 
	// 한번 더 그리는 식

	// 위와 같은 식으로, 삼각형 대신 선을 그리는
	// LINELIST, LINESTRIP도 있음

	Device::Get()->GetDeviceContext()->Draw(6, 0);
	// 세팅을 다 끝냈으면 
	// 삼각형이 보유하고 있는 정점의 개수만큼 도형을 그리라고 요청

	// 3 : 삼각형을 그리는 데 사용할 정점의 개수
	// 0 : 입력된 전체 정점들 중 몇번째 정점부터 사용할 것인지
	// 그 인덱스를 입력하는 부분
	// -> 전체 정점들 중 0번 인덱스부터 
	// 총 3개의 정점을 이용해 그림을 그려라

}

/*

과제 - 위에서 삼각형이나 사각형 등을 그려봤는데

한번 오늘 설명을 들은 걸 이용해서
삼각형, 사각형 외의 다른 도형을 한 번 그려볼 것

어떤 도형을 그릴지는 여러분 마음대로이나,
뭔가 이름을 가진 도형을 만들어볼 것(별 / 육망성 등)
꼭지점에 색을 전부 다르게 만들어서 보기 좋은 도형으로 구성해볼 것
