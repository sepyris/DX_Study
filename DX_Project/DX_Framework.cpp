// DX_Framework.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
/*
ImGui쪽에서마우스 입력시 발생하는 윈도우 메세지를 받아서 처리 하는 함수
간단히 말하면 ImGui전용 WndProc

함수 자체는 imgui_impl_win32라이브러리에 존재 하지만,
정작 해당 함수에서는 정의만 존재하고 선언은 되어이지 않아서
사용하려면 이 쪽 프로젝트에서 extern으로 선언 해야 함
그래야 이 함수에 대한 정의를 다른 파일에서 찾아 접근할수 있기 때문
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (ImGui_ImplWin32_WndProcHandler(hWnd, message, wParam, lParam)) {
        return true;
        //위에서 활성화한 ImGui쪽의 메세지 처리 함수에 먼저 데이터를 넣고
        //지금 발생한 메세지가 ImGui창과 상호작용하는 메세지 였다면
        //해당 작업을 처리하고 윈도우 프로시저 종료
        //아니라면 아래로 넘겨서 정상적인 윈도우 메세지 처리
    }
