...
VertexShader::VertexShader(wstring file_loc, UINT type, DWORD flags)
{
    ...
    case 2: {
        ...
        Device::Get()->GetDevice()->CreateInputLayout(layouts_UV, layoutSize,
            vertexBlob->GetBufferPointer(),
            vertexBlob->GetBufferSize(), &input_layout);
        break;
    }
...
VertexShader* VertexShader::GetInstance(wstring file_loc, UINT type)
{
    if (shader_data.count(file_loc) > 0) {
        //입력받은 파일 주소를 키값으로 가지는 데이터가 하나라도 존재한다면
        return shader_data[file_loc];
        //맵에 이미 저장되어있는 객체를 반환
    }
    VertexShader* instance = new VertexShader(file_loc, type);
    //만약 해당 파일 주소를 통해 만들어진적이 없다면 이자리에서 생성
    //생성한 객체를 맵에 등록

    shader_data[file_loc] = instance;

    return instance;//일단 주문받은 것은 반환 해주기

    //이 함수의 의의는 한번 생성된 VertexShader를 중복없이 여러객체에서 쓸수 있도록
    //데이터베이스에 생성해두고 꺼내서 쓰게 하기 위한 용도

    //->앞으로 애니메이션을위해 다루게 될 스프라이트 하나하나가 원활한 관리를위해 Frame이라는 클래스로서 관리하게 될것이며
    //그 프레임 하나하나가 서로다른 VertexShader를 이용하게 되기 때문
    //스프라이트 갯수에 따라서 엄청난 수량이 만들어질 것인데
    //그것들이 같은 셰이더를 이용함에도
    //각자 다른 인스턴스를 만들어 쓰는건 심각한 낭비
    //그런 낭비를 줄이기 위해 이런 작업을 함
}
